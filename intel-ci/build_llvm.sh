...
# Handle truncated SHA in llvm-project directory name
if [ ! -d "${LLVM_PROJECT_DIR}" ]; then
  LLVM_PROJECT_DIR_PATTERN="${LLVM_PROJECT_DIR}*"
  ACTUAL_DIR=$(ls -d ${LLVM_PROJECT_DIR_PATTERN} 2>/dev/null | head -n 1)
  if [ -n "${ACTUAL_DIR}" ] && [ -d "${ACTUAL_DIR}" ]; then
    echo "Creating symlink: ${ACTUAL_DIR} -> ${LLVM_PROJECT_DIR}"
    ln -s "${ACTUAL_DIR}" "${LLVM_PROJECT_DIR}"
  else
    echo "LLVM project directory not found"
    rm -r ${LLVM_INSTALL_DIR}
    exit 1
  fi
fi
